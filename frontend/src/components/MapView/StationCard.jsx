...
export function StationCardContent({ station }) {
  if (!station) return null;
  const { label, hex } = riskPresentation(station.level);

  return (
    <div className="station-popup-content">
      <div style={{ fontSize: 14.5, fontWeight: 600, color: '#FFFFFF', lineHeight: 1.25 }}>{station.name}</div>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 4 }}>
        <span style={{ fontSize: 9, letterSpacing: 0.4, color: 'var(--color-text-label)' }}>
          {station.code} · {station.sector.toUpperCase()}
        </span>
        <span style={{ fontSize: 9, color: 'var(--color-text-dim)', whiteSpace: 'nowrap' }}>
          {station.date ? formatDateEs(station.date) : '—'}
        </span>
      </div>
      <div
        style={{
          display: 'inline-block', marginTop: 9, padding: '4px 11px', borderRadius: 999,
          fontSize: 10, letterSpacing: 0.4, border: `1px solid ${hex}`,
          background: `${hex}2A`, color: hex,
        }}
      >
...
function Field({ label, value }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <span style={{ fontSize: 15, color: '#FFFFFF' }}>{value}</span>
    </div>
  );
}
